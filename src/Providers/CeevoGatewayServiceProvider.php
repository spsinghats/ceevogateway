...
namespace CeevoGateway\Providers;

use CeevoGateway\Methods\CeevoGatewayPaymentMethod;
use CeevoGateway\Helper\CeevoGatewayHelper;
...
class CeevoGatewayServiceProvider extends ServiceProvider
{
    /**
     * @param CeevoGatewayHelper $paymentHelper
     * @param PaymentMethodContainer $payContainer
     * @param Dispatcher $eventDispatcher
     */
    public function boot(
        CeevoGatewayHelper $paymentHelper,
        PaymentMethodContainer $payContainer,
        Dispatcher $eventDispatcher
        ) 
    {
         // Register the Invoice payment method in the payment method container
        $payContainer->register('plenty::CEEVO', CeevoGatewayPaymentMethod::class,
        ...
        $eventDispatcher->listen(GetPaymentMethodContent::class,
            function(GetPaymentMethodContent $event) use( $paymentHelper) {
                if($event->getMop() == $paymentHelper->getMop()) {
                    ...
                    $event->setValue( $translator->trans('CeevoGateway::error.errorInvalidParcelService'));
