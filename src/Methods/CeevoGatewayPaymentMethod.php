...
namespace CeevoGateway\Methods;
...
/**
 * Class CeevoGatewayPaymentMethod
 * @package CeevoGateway\Methods
 */
class CeevoGatewayPaymentMethod extends PaymentMethodService
{
    /**
    ...
    public function getName($lang='de')
    {
        ...
        $paymentMethodName = $trans->trans('CeevoGateway::PaymentMethod.name');
        if(strlen($paymentMethodName)){
            return $paymentMethodName;
        }
        $name = $this->config->get('CeevoGateway.name');
        if(strlen($name) > 0) {
            return $name;
        } 
    ...
    {
        $logo = $this->config->get('CeevoGateway.logo');
        if(strlen($logo) > 0) {
            return $logo;
        }
        /** @var Application */
        $app = pluginApp(Application::class);
        return $app->getUrlPath('CeevoGateway').'/images/logos/nachnahme.png';
    }
